...
export function renderWizardHtml(options: {
  ...
  const routeCards = options.controlMode ? `
    ...
    <section class="modes">
      <article class="mode"><div class="num">ROUTE / 01</div><h2>快速启动</h2><p>无需账号与域名。本机 Hub 通过一次性 Cloudflare 临时隧道上线。</p><button class="button primary" data-action="quick">生成临时连接</button></article>
      <article class="mode"><div class="num">ROUTE / 02</div><h2>深度配置</h2><p>填写固定域名，完成 Cloudflare 授权后自动创建 Named Tunnel 与 DNS。</p><button class="button" data-action="deep">配置固定域名</button></article>
    </section>`;
  ...
  const apkCard = options.apkUrl && options.apkQrSvg ? `
    <section class="apk">
      <div><div class="num">ANDROID / COMMUNITY</div><h2>Community APK</h2><p>扫码下载支持任意 HTTPS Hub 的开源版本；可与内部固定域名版本同时安装。</p><div class="actions"><a class="button" href="${escapeHtml(options.apkUrl)}" target="_blank" rel="noreferrer">下载 APK</a><button class="button" data-action="install-adb">ADB 一键安装</button></div></div>
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{background:var(--ink)}body{margin:0;min-height:100vh;color:var(--text);font-family:"Avenir Next","DIN Alternate","Segoe UI",sans-serif;background:radial-gradient(circle at 70% 15%,rgba(36,113,145,.18),transparent 35%),linear-gradient(145deg,#030506 0%,#081015 54%,#030506 100%);overflow-x:hidden}
    ...
    @media(max-width:760px){.shell{padding-top:26px}.mast{grid-template-columns:1fr}.sonar{order:-1;width:210px}.modes,.fields,.apk{grid-template-columns:1fr}.apkqr{width:170px}.mode{min-height:220px}.steps{grid-template-columns:1fr 1fr}.mast h1{font-size:52px}}
...
<body>
  ...
  <script>
    const csrf=${csrf};const out=document.getElementById('console');const deep=document.getElementById('deep');
    ...
    async function call(action,body={}){const requestId=crypto.randomUUID();document.querySelectorAll('button').forEach(b=>b.disabled=true);out.textContent='执行 '+action+'…';try{const response=await fetch('/api/'+action,{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json','x-dsh-csrf':csrf,'x-dsh-request-id':requestId},body:JSON.stringify(body)});const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));out.textContent=data.message||JSON.stringify(data,null,2);return data}catch(error){out.textContent='失败：'+error.message;throw error}finally{document.querySelectorAll('button').forEach(b=>b.disabled=false)}}
    document.addEventListener('click',event=>{const button=event.target.closest('[data-action]');if(!button)return;const action=button.dataset.action;if(action==='deep'){deep.hidden=false;deep.scrollIntoView({behavior:'smooth'});return}if(action==='quick')call('quick');else if(action==='save-domain')call('named/domain',values());else if(action==='check-ns')call('named/check-ns',values());else if(action==='authorize')call('named/authorize',values());else if(action==='provision')call('named/provision',values());else if(action==='install-adb')call('apk/install-adb');});
